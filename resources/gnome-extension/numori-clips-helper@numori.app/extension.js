...
export default class NumoriClipsHelper extends Extension {
  ...
  _startClipboardMonitoring() {
    ...
    this._selectionChangedId = selection.connect('owner-changed',
  ...
  _bindKey(name) {
    try {
      ...
      log(`[Numori Clips Helper] Failed to bind key ${name}: ${e}`);
    }
  }
  ...
  _setShortcut(name, accelerator) {
    try {
      ...
      log(`[Numori Clips Helper] Failed to set shortcut ${name}: ${e}`);
      return false;
    }
  }
  ...
  _showWindow(wmClass, x, y, width, height, titleHint) {
    ...
    const timerId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, duration / steps, () => {
      step++;
      const t = step / steps;
      // Ease out cubic
