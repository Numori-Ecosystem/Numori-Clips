...
export const IS_WAYLAND = process.env.XDG_SESSION_TYPE === 'wayland' || process.env.WAYLAND_DISPLAY != null
...
function tryElectronCursor() {
  ...
  try { return screen.getDisplayNearestPoint(screen.getCursorScreenPoint()) } catch { return null }
}
...
function tryKdeCursorPosition() {
  ...
  try {
    ...
    const scriptId = execSync(`qdbus org.kde.KWin /Scripting org.kde.kwin.Scripting.loadScript "${scriptPath}"`, { encoding: 'utf8', timeout: 2000 }).trim()
    execSync(`qdbus org.kde.KWin /Scripting/Script${scriptId} org.kde.kwin.Script.run`, { encoding: 'utf8', timeout: 2000 })
    const journal = execSync('journalctl --user -t kwin_wayland_wrapper -t kwin_wayland --since "2 seconds ago" --no-pager -o cat 2>/dev/null || true', { encoding: 'utf8', timeout: 2000 })
    try { execSync(`qdbus org.kde.KWin /Scripting/Script${scriptId} org.kde.kwin.Script.stop`, { encoding: 'utf8', timeout: 1000 }) } catch {}
...
export function isGnomeExtensionAvailable() {
  ...
  try { gdbus('GetCurrentMonitor'); return true } catch { return false }
}

export function isGnomeExtensionInstalled() {
  if (!IS_GNOME) return false
  const dir = join(homedir(), '.local', 'share', 'gnome-shell', 'extensions', 'numori-clips-helper@numori.app')
...
export function installGnomeExtension() {
  ...
  try {
    ...
    const src = join(__dirname, '..', 'resources', 'gnome-extension', 'numori-clips-helper@numori.app')
    const dest = join(homedir(), '.local', 'share', 'gnome-shell', 'extensions', 'numori-clips-helper@numori.app')
    cpSync(src, dest, { recursive: true })
    try { execSync('gnome-extensions enable numori-clips-helper@numori.app', { stdio: 'pipe', timeout: 3000 }) } catch {}
    return true
  } catch (e) { console.error('[Numori Clips] Failed to install extension:', e); return false }
}

export function enableGnomeExtension() {
  if (!IS_GNOME) return false
  try { execSync('gnome-extensions enable numori-clips-helper@numori.app', { stdio: 'pipe', timeout: 3000 }); return true }
  catch { try { execSync("gdbus call --session --dest org.gnome.Shell.Extensions --object-path /org/gnome/Shell/Extensions --method org.gnome.Shell.Extensions.EnableExtension 'numori-clips-helper@numori.app'", { stdio: 'pipe', timeout: 3000 }); return true } catch { return false } }
}
...
export function toGtkAccelerator(shortcut) {
  ...
  const key = parts.pop()
  const mods = parts.map(m => ({ Super: '<Super>', Ctrl: '<Control>', Alt: '<Alt>', Shift: '<Shift>' }[m] || `<${m}>`)).join('')
